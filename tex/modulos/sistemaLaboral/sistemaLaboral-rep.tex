...
\begin{RepFormal}{estrP}{e}
	...
	\repItem{
		\#(e.gremios) = longuitud(e.gruposDeAliados)
	}{\ly}
	\repItem{
		longuitud(e.gruposDeAliados) > 0 \entonces esta?(0,e.gruposDeAliados) \ly ((\paratodof{nat}{n})\, (0 < n \ly esta?(n,e.gruposDeAliados)) \entonces esta?(n-1,e.gruposDeAliados))
	}{}
\end{RepFormal}
...
\begin{FunAbsDescriptiva}{e}{estrS}{si}{sistemaLaboral}
	...
	\absItem{(\paratodof{g}{gremio}) g \en gremios(si) \entoncesL aliados(g, si) \igobs obtenerConjuntoDeAliados(e.gruposDeAliados, e.gremios,g)}{}
...
{
	if(vacio?(gremios)) then \\
	\hspace*{10pt}$\emptyset$	\\
	else \\
		\hspace*{10pt}if(iesimo(id(dameUno(gremios), indiceAliados) \igobs iesimo(id(g), indiceAliados) then \\
			\hspace*{20pt}dameUno(gremios) $\cup$ obtenerConjuntoDeAliados(sinUno(gremios), g) \\
		\hspace*{10pt}else \\
			\hspace*{20pt}obtenerConjuntoDeAliados(sinUno(gremios), g) \\
		\hspace*{10pt}fi \\
	fi \\
}