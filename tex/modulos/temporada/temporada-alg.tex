...
\algoritmoI{GremioConflictivo}{\param{in}{tmp}{estrT}}{gremio}
{
	...
	\state \var{nat}{maxAperturas} \asig Obtener\#Aperturas(tmp, gremioConflictivo)				\inlineC{1}
	...
	\while{HaySiguiente(itGremios)}																\inlineC{1}
		...
		\state \var{nat}{\#aperturas} \asig Obtener\#Aperturas(tmp, Siguiente(itGremios))		\inlineC{1}

		\If{maxAperturas \menor \#aperturas}							\inlineC{1}

			\state maxAperturas \asig \#aperturas						\inlineC{1}
...
{\#gremios}
{Obtiene el gremio con cantidad m\'axima de aperturas. Para esto se efectua una busqueda lineal sobre el conjunto de gremios, por lo que esta funci\'on posee un costo temporal O(\#gremios).}
